...
from otel_observability import flush_otel, get_tracer, should_trace_http_path
...
def handler(event, context):
    try:
        ...
        if not should_trace_http_path(path):
            return _asgi_handler(event, context)
        with tracer.start_as_current_span("api.lambda_request", kind=SpanKind.SERVER) as span:
            if span is not None:
