...
# Persist data under api/data by default so runtime is self-contained in api/.
_THIS_FILE = Path(__file__).resolve()
_API_ROOT = _THIS_FILE.parents[1]
_DEFAULT_DATA_DIR = _API_ROOT / "data"
DATA_DIR = Path(os.getenv("TRADER_DATA_DIR", str(_DEFAULT_DATA_DIR))).resolve()
DATA_DIR.mkdir(parents=True, exist_ok=True)
DB = str(DATA_DIR / "accounts.db")
